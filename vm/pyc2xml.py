...
FILE can be a .pyc, .pyo, .py or .xml file.
Files are handled by their extension:
(i) .py* files are disassembled to .xml
(ii) .xml is assembled to .pyc

A .py file is (internally) compiled to bytecode, and then this
bytecode is disassembled into XML, missing the header information.
    """)
